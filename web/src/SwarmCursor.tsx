...
const SwarmCursor: React.FC<SwarmCursorProps> = ({
  color = '#ffffff',
  accentColor = '#ffffff',
  count = 10,
  size = 10,
  ...
  useEffect(() => {
    ...
    window.addEventListener('pointerenter', onMove as any, { passive: true });
    window.addEventListener('pointerleave', onLeave);
    window.addEventListener('pointerdown', onDown as any);
    ...
    return () => {
      ...
      window.removeEventListener('pointerenter', onMove as any);
  ...
  return (
    <div ref={containerRef} className={`absolute inset-0 pointer-events-none z-50 ${className}`.trim()} style={style} {...rest}>
      {children ? (
